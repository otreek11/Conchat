...
function Chat() {
  ...
  const fetchCurrentUser = async (): Promise<CurrentUser | null> => {
    ...
    try {
      ...
      const userResponse = await fetch(`${BASE_URL}/user/id/${userUuid}`, {
        headers: {
          'Authorization': `Bearer ${token}`,
        },
      });
    ...
    try {
      ...
      const membersWithInfo = await Promise.all(
        data.members.map(async (member: any) => {
          const userResponse = await fetch(`${BASE_URL}/user/id/${member.user_id}`, {
      ...
      try {
        ...
        if (event.type === 'MESSAGE_NEW') {
          const newMessage: Message = {
            uuid: event.payload.message_id || uuidv4(),
